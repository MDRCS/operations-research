...
" This library provide some features extended from operations research tools library and improved and adapted to our need to elaborate easily the model "

"Utility Function to Create an Appropriate Solver Instance : " \

"newSolver(name, integer=False) -> Intialization of the solver" \

"name is of the problem that you are trying to solve" \

"if the integer is false that mean that the solver that we are going to use is linear programming " \

"else the solver is  MIXED INTEGER PROGRAMMING "
...
"SolVal is a function that get the solver in the parameter "


# def Simple_SolVal(x):
#   return [e.SolutionValue() for e in x]

def Simple_SolVal(x):

  if type(x) is not list:
    if x is None:
      # print('x',x.SolutionValue())
      return 0

    else:
      if isinstance(x,(int,float)):
        # print('isinstance(x,(int,float))',x.SolutionValue())
        return x

      else:

        if x.Integer() is False:
          # print('x.Integer() is False',x.SolutionValue())
          return x.SolutionValue()

        else:
          # print('x.Integer() is True',x.SolutionValue())
